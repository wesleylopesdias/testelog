var REPORTS = {
	...
	report_service_status:"#report-service-status",
	...
	check_auto_load:function() {
		...
		if(auto_load) {
			var month = UTIL.get_param_by_name("month");
			if(month) {
	...
	dates:{
		...
		setup_dates:function() {
			...
			var end_month_moment = moment().endOf("year");
			while(end_month_moment > start_month_moment) {
				var selected = "";
				if(end_month_moment.format(REPORTS.dates.format) == moment().startOf("month").format(REPORTS.dates.format)) selected = " selected=\"selected\"";
				output += "<option value=\"year\"" + selected + " data-month=\"" + end_month_moment.startOf("month").format(REPORTS.dates.format) + "\">" + end_month_moment.format(REPORTS.dates.display_format); + "</option>";
	...
	build_report_url:function(report_format) {
		var url = PAGE_CONSTANTS.BASE_URL + "reports/topservices." + report_format + "?";
		
		var group = $(REPORTS.report_group).val();
		var month = $(REPORTS.report_date_range + " option:selected").data("month");
		var status = $(REPORTS.report_service_status).val();
	...
	sort_data_for_chart:function(data) {
		...
		for(var i=0; i < data.length; i ++) {
			var record = data[i];
			categories.push(record.serviceName);
		...
		return { categories:categories, series:series, devices:devices, service_ids:service_ids, avg_prices:avg_prices, margins:margins };
	},
	        ...
	        tooltip: {
	            ...
	            pointFormatter: function() {
	            	...
	            	$.each(series, function(i, s) { 
	            		str += '<tr><td style="color:' + series[0].color + ';padding:0;">' + series[0].name + ': </td>' + '<td style="padding:0; text-align:right;"><b>' + UTIL.convert_currency_for_ui(s.data[index].y) + '</b></td></tr>';
	            		str += '<tr><td style="color:' + series[0].color + ';padding:0;">Device Count:</td><td style="padding:0; text-align:right;"><b>' + UTIL.convert_number_for_ui(data.devices[index]) + '</b></td></tr>';
	                });
	            	return str;
	            },
	            footerFormat: '</table>',*/
	                ...
	                point: {
	                    events: {
	                        click: function () {
	                        	...
	                        	var url = "/si/reports/revenue?auto=true&sid=" + osp_id + "&year=" + year;
	...
	build_table:function(data) {
		...
		var footer_output = "<tr class=\"total-row\"><td></td>";
