...
public class RevenueByServiceReportDocumentBuilder extends BaseDocumentBuilder {
    ...
    public RevenueByServiceReportDocumentBuilder(MessageSource messageSource, Locale locale) {
    ...
    public XSSFWorkbook buildRevenueByServiceReportExportSpreadsheet(List<RevenueByServiceRecord> report) {
        Integer rowidx = 1;
        writeReportData(rowidx, report);
        return this.workbook;
    }

    private void writeReportData(Integer rowidx, List<RevenueByServiceRecord> report) {
        //write out header
        RevenueByServiceRecord firstRecord = report.get(0);
        rowidx = writeReportDataHeader(rowidx, firstRecord);

        for (RevenueByServiceRecord reportRecord : report) {
    ...
    private int writeReportDataHeader(Integer rowidx, RevenueByServiceRecord record) {
    ...
    private int writeReportDataRow(Integer rowidx, RevenueByServiceRecord record) {
        ...
        header = sheet1.createRow(rowidx++);
        int colidx = 0;
        header.createCell(colidx).setCellValue(record.getBusinessModel());
        header.getCell(colidx++).setCellStyle(style);
